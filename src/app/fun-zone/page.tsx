...
export default function FunZone() {
  ...
  const chartData = salamiRecords?.slice(0, 10).map(r => ({ name: r.fromPerson || "Anonymous", amount: r.amount })) || []
    ...
    <div className="min-h-screen bg-background islamic-pattern pb-20">
      ...
      <main className="max-w-7xl mx-auto px-4 py-16">
        <header className="text-center mb-16 space-y-6">
          ...
          <h1 className="text-5xl lg:text-8xl font-black text-primary dark:text-white tracking-tighter">Viral Engagement</h1>
          <p className="text-xl text-muted-foreground font-medium max-w-2xl mx-auto leading-relaxed">
            The heart of EidSpark entertainment. Generate excuses, discover your personality, and join the selfie contest!
          </p>
        </header>
        ...
        <Tabs defaultValue="excuse" className="w-full space-y-12">
          <TabsList className="grid w-full grid-cols-2 lg:grid-cols-5 h-auto lg:h-16 p-1 bg-white/50 dark:bg-slate-900/50 backdrop-blur-md rounded-[2rem] shadow-xl border border-primary/10 overflow-hidden">
            ...
            <TabsTrigger value="counter" className="rounded-[1.5rem] font-black gap-2 py-3"><Wallet className="w-4 h-4" /> Salami Counter</TabsTrigger>
            ...
            <div className="grid lg:grid-cols-12 gap-10">
              ...
              <Card className="lg:col-span-8 border-none shadow-2xl rounded-[3rem] bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-10 flex flex-col">
                <div className="flex items-center justify-between mb-10">
                  <CardTitle className="text-2xl font-black flex items-center gap-3"><ChartBar className="w-6 h-6 text-primary" /> Tracking Visualization</CardTitle>
                </div>
          ...
          {/* 4. Nearest Jamaat Alert */}
          <TabsContent value="jamaat" className="animate-in fade-in duration-700 text-center">
             <Card className="max-w-4xl mx-auto border-none shadow-2xl rounded-[3rem] overflow-hidden bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-12 space-y-8">
                <div className="w-20 h-20 bg-primary/5 rounded-[2.5rem] flex items-center justify-center mx-auto text-primary">
                  <MapPin className="w-10 h-10 animate-bounce" />
                </div>
                <div className="space-y-3">
                  <h3 className="text-4xl font-black text-primary">Find Eid Prayers Near You</h3>
                  <p className="text-muted-foreground text-lg max-w-xl mx-auto">We use your browser's GPS to find the nearest mosques and community verified prayer times.</p>
                </div>
                <div className="pt-6">
                  <Button className="h-20 px-12 rounded-[2rem] emerald-gradient text-white font-black text-2xl shadow-2xl hover:scale-105 transition-transform" asChild>
                    <a href="/tools/jamaat-finder">
                      Launch Live Map <ChevronRight className="ml-2 w-6 h-6" />
                    </a>
                  </Button>
                </div>
                <div className="pt-10 flex flex-wrap justify-center gap-8 border-t border-primary/5">
                   ...
                   <div className="flex items-center gap-3">
                     ...
                     <span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Community Verified</span>
                   </div>
                </div>
             </Card>
          </TabsContent>
            ...
            <div className="grid lg:grid-cols-12 gap-10">
              <Card className="lg:col-span-4 border-none shadow-2xl rounded-[3rem] overflow-hidden bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl">
                <div className="emerald-gradient p-10 text-white text-center">
                  ...
                  <CardTitle className="text-3xl font-black">Join The Contest</CardTitle>
                  <p className="text-white/60 text-xs font-bold uppercase mt-2">Win the "EidSpark Star" Badge</p>
                </div>
                <CardContent className="p-10 space-y-6">
                  {!user ? (
                    <Button className="w-full h-16 rounded-2xl emerald-gradient font-black" asChild><a href="/login">Login to Upload</a></Button>
                  ) : (
                    <div className="space-y-6">
                      <div className="space-y-2">
                        <Label className="text-[10px] font-black uppercase ml-1">Photo URL (Demo)</Label>
                        <Input value={selfieUrl} onChange={e => setSelfieUrl(e.target.value)} placeholder="https://..." className="h-14 rounded-2xl" />
                      </div>
                      <div className="space-y-2">
                        <Label className="text-[10px] font-black uppercase ml-1">Fun Caption</Label>
                      ...
                      <Button onClick={uploadSelfie} disabled={isUploading || !selfieUrl} className="w-full h-16 rounded-2xl gold-gradient text-primary font-black text-xl shadow-xl">
                        {isUploading ? <Loader2 className="animate-spin" /> : "Post My Selfie"}
                      </Button>
                    </div>
                  )}
                </CardContent>
              ...
              <div className="lg:col-span-8 space-y-8">
                <div className="flex items-center justify-between px-4">
                  <h3 className="text-2xl font-black text-primary flex items-center gap-3"><Trophy className="w-6 h-6 text-secondary fill-secondary" /> Leaderboard</h3>
                  ...
                  {topSelfies && topSelfies.length > 0 ? topSelfies.map((s, idx) => (
                    <Card key={s.id} className="border-none shadow-xl rounded-[2.5rem] overflow-hidden bg-white group hover:-translate-y-2 transition-all">
                      <div className="aspect-square relative overflow-hidden bg-slate-100">
                        <Image src={s.imageUrl} alt="Contest Selfie" fill className="object-cover group-hover:scale-105 transition-transform duration-500" />
                      ...
                      <CardContent className="p-6 flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Posted by User</p>
                        </div>
