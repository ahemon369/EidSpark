      ...
      <div className="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
        <div className="text-center mb-12 space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-black uppercase tracking-widest border border-primary/20">
            ...
            <span>Eid Design Studio</span>
          </div>
          <h1 className="text-4xl lg:text-6xl font-black text-primary tracking-tight">Selfie Editor</h1>
        </div>
            ...
            <Card className="border-none shadow-xl rounded-[2.5rem] overflow-hidden bg-white/80 backdrop-blur-xl border border-white/20">
              <Tabs defaultValue="image" className="w-full">
                <TabsList className="grid w-full grid-cols-4 h-16 bg-primary/5 rounded-none border-b border-primary/10">
                  <TabsTrigger value="image" className="data-[state=active]:bg-white rounded-none border-r border-primary/5"><ImageIcon className="w-4 h-4" /></TabsTrigger>
                ...
                <CardContent className="p-6 space-y-6">
                  ...
                  <TabsContent value="image" className="space-y-6 mt-0">
                    <div className="space-y-4">
                      <Label className="text-xs font-black text-muted-foreground uppercase tracking-widest">1. Upload Selfie</Label>
                      <input type="file" accept="image/*" className="hidden" id="editor-upload" onChange={handleImageUpload} />
                      <label htmlFor="editor-upload" className="flex flex-col items-center justify-center h-40 border-2 border-dashed border-primary/20 rounded-2xl cursor-pointer hover:bg-primary/5 transition-all">
                        {originalImage ? (
                    ...
                    <div className="space-y-4 pt-4 border-t border-primary/5">
                      <Label className="text-xs font-black text-muted-foreground uppercase tracking-widest">2. AI Background</Label>
                      ...
                      <Button 
                        ...
                        className="w-full emerald-gradient rounded-xl font-bold"
                      >
                        {isGenerating ? "Transforming..." : "Apply AI Scene"}
                      </Button>
                    </div>
                  </TabsContent>
                  ...
                  <TabsContent value="text" className="space-y-6 mt-0">
                    <div className="space-y-2">
                      <Label className="text-xs font-bold uppercase">Eid Message</Label>
                  ...
                  <TabsContent value="stickers" className="space-y-6 mt-0">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      <Button variant={showLanterns ? "default" : "outline"} className="h-12 rounded-xl justify-between px-4 font-bold" onClick={() => setShowLanterns(!showLanterns)}>
                        <span>Lanterns</span>
                        {showLanterns ? <Sparkles className="w-4 h-4" /> : null}
                      </Button>
                      <Button variant={showStars ? "default" : "outline"} className="h-12 rounded-xl justify-between px-4 font-bold" onClick={() => setShowStars(!showStars)}>
                        ...
                        {showStars ? <Sparkles className="w-4 h-4" /> : null}
                      </Button>
                      <Button variant={showCrescent ? "default" : "outline"} className="h-12 rounded-xl justify-between px-4 font-bold" onClick={() => setShowCrescent(!showCrescent)}>
                        ...
                        {showCrescent ? <Sparkles className="w-4 h-4" /> : null}
                      </Button>
                    </div>

                    <Label className="text-xs font-black text-muted-foreground uppercase tracking-widest pt-4 block">Frame Style</Label>
                    <div className="grid grid-cols-1 gap-2">
                      {frames.map((frame) => (
                        <button
                          ...
                          className={cn(
                            "h-12 rounded-xl border-2 px-4 flex items-center gap-3 transition-all",
                        ...
                        >
                          ...
                          <span className="text-xs font-bold uppercase">{frame.name}</span>
                        </button>
                      ))}
                    </div>
                  </TabsContent>
            ...
            <div className="relative w-full aspect-square max-w-xl bg-white rounded-[3rem] overflow-hidden shadow-2xl border-8 border-white group">
              ...
              {!(originalImage || aiGeneratedImage) && (
                <div className="absolute inset-0 bg-white/40 backdrop-blur-sm flex flex-col items-center justify-center p-12 text-center space-y-6 pointer-events-none">
                  ...
                  <p className="font-black text-primary/40 uppercase tracking-widest">Start by uploading a selfie</p>
                </div>
              )}
            </div>
