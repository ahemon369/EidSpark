  ...
  return (
    <nav className={cn(
        ...
        <div className="flex justify-between items-center h-full">
          <Link href="/" className="flex items-center gap-3 group z-50">
            <div className="relative h-8 w-8 transition-transform group-hover:scale-110">
               {logo?.imageUrl && (
                 <Image src={logo.imageUrl} alt="EidSpark" width={32} height={32} className="object-contain" priority />
               )}
            </div>
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={cn(
                  ...
                  pathname === item.href ? "text-primary bg-primary/5" : "text-slate-600 hover:text-primary hover:bg-slate-50"
                )}
              >
          ...
          <div className="hidden lg:flex items-center gap-4 z-50">
            {user && !loading && (
              <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-secondary/10 border border-secondary/20 shadow-sm">
                ...
                <span className="text-xs font-black text-primary tracking-tighter">{totalPoints} XP</span>
              </div>
            )}
            ...
            {loading ? (
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent className="w-64 p-2 rounded-2xl mt-2" align="end">
                  ...
                  {username && (
                    <DropdownMenuItem asChild className="rounded-xl font-bold">
                      <Link href={`/profile/${username}`}>Public Profile</Link>
                    </DropdownMenuItem>
                  )}
                  <DropdownMenuItem asChild className="rounded-xl font-bold">
              ...
              <Button asChild className="emerald-gradient text-white rounded-full font-black px-8 h-11 shadow-xl hover:scale-105 transition-all">
      ...
      <div className={cn(
        ...
        <button onClick={() => setIsOpen(false)} className="absolute top-6 right-6 p-2 rounded-xl text-primary bg-primary/5">
          <X className="h-6 w-6" />
        </button>
